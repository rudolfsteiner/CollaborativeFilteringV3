...
class RecommandationModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.user_itemcount_placeholder = tf.placeholder(tf.int32, shape = [None, ], name = "user_rated_item_count")
        self.user_rated_item_placeholder = tf.placeholder(tf.int32, shape = [None, None], name = "y_placeholder")
    ...
    def add_prediction_op(self):
        with tf.device("/cpu:0"):
            ...
            self.Y = tf.get_variable("Y", shape = [self.config.n_items, self.config.n_f], 
                                   initializer =  weight_initializer)
        ...
        with tf.device(self.config.device):             
            ...
            ru_by_user = tf.pow(tf.to_float(self.user_itemcount_placeholder), -0.5)
