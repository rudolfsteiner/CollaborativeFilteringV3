...
class Config(object):
    ...
    item_bin_size = 60
    batch_size = 2048
    weight_filename = "data\\weights\\recommanding_with_user_day_drifting.weight"
    device="/cpu:0"
...
class RecommandationModel(Model):
 ...
 + tf.nn.l2_loss(self.BI) +  tf.nn.l2_loss(self.WPU) + tf.nn.l2_loss(self.WPI)) + 0.5 * self.config.lamb5 * tf.nn.l2_loss(self.bias_item_binvalue) + 0.5*self.config.lamb6*tf.nn.l2_loss(self.bias_user_tvalue) + 0.5 * self.config.lamb7 * tf.nn.l2_loss(self.BTDay)
