...
class RecommandationModel(Model):
    ...
    def add_prediction_op(self):
        ...
        self.test_pred = pred #tf.add(self.global_mean_placeholder, bias_user)
    ...
    def add_loss_op(self, pred):
        ...
        """        
        loss =  tf.nn.l2_loss(tf.subtract(pred ,self.rank_placeholder)) + 0.5*self.config.lamb4*(tf.nn.l2_loss(self.BU) + tf.nn.l2_loss(self.BI)) #+tf.nn.l2_loss(self.WPU) + tf.nn.l2_loss(self.WPI)) + 0.5*self.config.lamb5*tf.nn.l2_loss(self.bias_item_binvalue) + 0.5*self.config.lamb6*tf.nn.l2_loss(self.bias_user_tvalue)
